...
public sealed class PdfService : IPdfService
{
    ...
    private static List<PdfTableColumn> BuildStandardColumnList(DocumentLineColumnVisibility v, string qtyHeader)
    {
        ...
        if (v.ShowDesignation)
            columns.Add(new PdfTableColumn("Désignation", 2.0f, PdfTextAlignment.Start));
        if (v.ShowQuantite)
            columns.Add(new PdfTableColumn(qtyHeader, 0.75f, PdfTextAlignment.Center));
        if (v.ShowConditionnement)
            columns.Add(new PdfTableColumn("Ute", 0.38f, PdfTextAlignment.Center));
        if (v.ShowPuHt)
            columns.Add(new PdfTableColumn("PU HT", 0.70f, PdfTextAlignment.Center));
        if (v.ShowTva)
            columns.Add(new PdfTableColumn("Tva", 0.38f, PdfTextAlignment.Center));
        if (v.ShowRemise)
            columns.Add(new PdfTableColumn("Rem. %", 0.48f, PdfTextAlignment.Center));
        if (v.ShowMontantHt)
            columns.Add(new PdfTableColumn("Mnt HT", 0.90f, PdfTextAlignment.Center));
        if (v.ShowMontantTtc)
            columns.Add(new PdfTableColumn("Mnt TTC", 0.90f, PdfTextAlignment.Center));
        return columns;
    }
